...
public partial class manageproduct : System.Web.UI.Page
{
    ...
    //                dtPDFPath.Rows.Add(@"http://software.mgcagro.com/uploads/" + PDFName + ".pdf");

    //                txtPath.Text = "http://software.mgcagro.com/uploads/" + PDFName + ".pdf";
        ...
        else
        {
            if (ListBox1.SelectedIndex == 0)
            {
                ...
                try
                {
                    ...
                    PdfPTable tableCategory = new PdfPTable(3);
                    iTextSharp.text.Image myImg = iTextSharp.text.Image.GetInstance(Server.MapPath("Uploads/navrang.png"));
                    myImg.Alignment = iTextSharp.text.Image.ALIGN_CENTER;
                    myImg.ScaleAbsolute(220f, 75f);
                    ...
                    {
                        a[i] = " NAVRANG         NAVRANG          NAVRANG         NAVRANG         NAVRANG         NAVRANG ";
                    }
    ...
    */
    ...
    //protected void ddlSelectChecked_SelectedIndexChanged(object sender, EventArgs e)
    //{
    //    DataTable dtCategory = new DataTable();
    //    int checkmark = Convert.ToInt16(ddlSelectChecked.SelectedValue.ToString());
    //    if (Session["dtProduct"] != null)
    //        dtCategory = (DataTable)Session["dtProduct"];
    //    if (ddlSelectChecked.SelectedIndex == -1)
    //    {
    //        repProduct.DataSource = dtCategory;
    //        repProduct.DataBind();
    //    }
    //    else
    //    {
    //        DataView dataView = dtCategory.DefaultView;

    //        String expression = "isactive ="+checkmark;
    //        dataView.RowFilter = expression;

    //        /* if (!string.IsNullOrEmpty(docname))
    //        if (!String.Equals(docname, "ALL"))
    //        {
    //        dataView.RowFilter = expression;
    //        }
    //        * */
    //        repProduct.DataSource = dataView;
    //        repProduct.DataBind();
